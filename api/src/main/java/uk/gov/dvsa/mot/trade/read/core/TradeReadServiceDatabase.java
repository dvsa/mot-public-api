...
public class TradeReadServiceDatabase implements TradeReadService
{
  ...
  public uk.gov.dvsa.mot.trade.api.Vehicle getLatestMotTestByRegistration( String registration )
  {
    ...
    List<Vehicle> lVehicle = vehicleReadService.findByRegistration( registration ) ;

    if ( !( lVehicle == null ) && !lVehicle.isEmpty() )
    {
  ...
  public List<DisplayMotTestItem> getMotTestsByRegistrationAndMake( String registration, String make )
  {
    ...
    if ( vehicle != null )
    {
      List<MotTest> lMotTest = motTestReadService.getMotTestsByVehicleId( vehicle.getId() ) ;

      for ( MotTest mottest : lMotTest )
      {
        DisplayMotTestItem itemToAdd = mapMotTestToDisplayMotTestItem( mottest, vehicle ) ;
        lDisplayMotTestItem.add( itemToAdd ) ;
      }
    }
  ...
  private DisplayMotTestItem mapMotTestToDisplayMotTestItem( MotTest motTest, Vehicle vehicle )
  {
    ...
    displayMotTestItem.setMake_name( vehicle.getMake() ) ;
    displayMotTestItem.setModel_name( vehicle.getModel() ) ;
    displayMotTestItem.setFuel_type( vehicle.getFuelType() ) ;
    if ( vehicle.getFirstUsedDate() != null )
    {
      displayMotTestItem.setFirst_used_date( sdfDate.format( vehicle.getFirstUsedDate() ) ) ;
    }
    displayMotTestItem.setPrimary_colour( vehicle.getPrimaryColour() ) ;
    displayMotTestItem.setRegistration( vehicle.getRegistration() ) ;
    if ( motTest.getCompletedDate() != null )
    {
      displayMotTestItem.setCompleted_date( sdfDate.format( motTest.getCompletedDate() ) ) ;
    }
    if ( motTest.getExpiryDate() != null )
    {
      displayMotTestItem.setExpiry_date( sdfDate.format( motTest.getExpiryDate() ) ) ;
    }

    displayMotTestItem.setMot_test_number( String.valueOf( motTest.getNumber() ) ) ;
    displayMotTestItem.setTest_result( motTest.getStatus() ) ;
    displayMotTestItem.setOdometer_value( String.valueOf( motTest.getOdometerReadingValue() ) ) ;
    displayMotTestItem.setOdometer_unit( motTest.getOdometerReadingUnit() ) ;
