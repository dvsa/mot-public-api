...
public class VehicleV7ResponseMapper extends VehicleResponseMapper {
    ...
    private String getRfrType(RfrAndAdvisoryItem rfrAndAdvisoryItem) {
        if (isPresentAndNotFailed(rfrAndAdvisoryItem)) {
            if (isEmptyOrPreEu(rfrAndAdvisoryItem.getDeficiencyCategoryCode())) {
    ...
    private boolean isPresentAndNotFailed(RfrAndAdvisoryItem rfrAndAdvisoryItem) {
        return !StringUtils.isNullOrEmpty(rfrAndAdvisoryItem.getType()) && rfrAndAdvisoryItem.getType().equals(TEST_TYPE_FAIL);
    }

    private boolean isEmptyOrPreEu(String deficiencyCode) {
        return StringUtils.isNullOrEmpty(deficiencyCode) || deficiencyCode.equals(RFR_DEFICIENCY_CATEGORY_PRE_EU_CODE);
    }
}
