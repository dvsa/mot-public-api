...
public class VehicleReadServiceDatabase implements VehicleReadService
{
  ...
  {
    List<Vehicle> lVehicle = vehicleReadDao.getVehicleByFullRegistration( registration ) ;

    return mapVehiclesSQLtoJSON( lVehicle ) ;
  }

  @Override
  public List<uk.gov.dvsa.mot.vehicle.api.Vehicle> getVehiclesByPage( int offset, int limit )
  {
    List<Vehicle> lVehicle = vehicleReadDao.getVehiclesByPage( offset, limit ) ;
  ...
  private List<uk.gov.dvsa.mot.vehicle.api.Vehicle> mapVehiclesSQLtoJSON( List<Vehicle> lStoredVehicle )
  {
    if ( lStoredVehicle == null )
    {
      return Arrays.asList() ;
    }

    List<uk.gov.dvsa.mot.vehicle.api.Vehicle> lVehicle = new ArrayList<>() ;

    for ( Vehicle vehicle : lStoredVehicle )
    {
      lVehicle.add( mapVehicleSQLtoJSON( vehicle ) ) ;
    }
