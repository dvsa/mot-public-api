...
public class TradeReadSql
{
  ...
  static final String JOIN_MOT_TEST_CURRENT_RFR_MAP = "LEFT JOIN `mot2`.`mot_test_current_rfr_map` AS `rfrmap` ON `rfrmap`.`mot_test_id` = `mot_test`.`id` " ;
  static final String FROM_MOT_TEST_HISTORY = "FROM    `mot2`.`mot_test_history` AS `mot_test` " ;
  static final String JOIN_MOT_TEST_HISTORY_RFR_MAP = "LEFT JOIN `mot2`.`mot_test_history_rfr_map` AS `rfrmap` ON `rfrmap`.`mot_test_id` = `mot_test`.`id` " ;
  ...
  static final String JOIN_MOT_TEST_AND_RFR_MAP_DIMENSIONS =
    ...
    + "LEFT JOIN `mot2`.`rfr_language_content_map` AS `rfrl` ON `rfrl`.`rfr_id` = `rfr`.`id` AND `rfrl`.`language_type_id` = `l`.`id` "
    + "LEFT JOIN `mot2`.`test_item_category` AS `ti` ON `rfr`.`test_item_category_id` = `ti`.`id` "
    + "LEFT JOIN `mot2`.`ti_category_language_content_map` AS `timap` ON `timap`.`test_item_category_id` = `rfr`.`test_item_category_id` AND `timap`.`language_lookup_id` = `l`.`id` "
  ...
  static final String ORDER_BY_VEHICLE_COMPLETED_MOT_TEST = "ORDER BY `vehicle_id` ASC, `mot_test_completed_date` DESC, `mot_test_id` DESC " ;
