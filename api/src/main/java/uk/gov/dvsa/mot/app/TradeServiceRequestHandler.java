...
public class TradeServiceRequestHandler extends AbstractRequestHandler
{
   ...
   * class.
   *
   * @param tradeReadService
   *          an instance of something which implements {@link TradeReadService}
   */
  @Inject
   ...
   *
   * @param request
   *          a {@link TradeServiceRequest} which describes the parameters to
   *          search for MOT tests by. This method only uses query parameters,
   *          and the valid options are: vehicleId, number (MOT test number),
   *          registration and make (must be specified together), date and page,
   *          or page.
   * @param context
   *          AWS Lambda context object
   * @return A list of {@link Vehicle}, with each vehicle populated with its MOT
   *         tests matching the search parameters.
   * @throws TradeException
   *           Under various error conditions, including no tests found. It is
   *           expected that the surrounding integration will interpret this
    ...
    try
    {
      ...
      if ( request.getQueryParams().getVehicleId() != null )
      {
        ...
        if ( vehicles.isEmpty() )
        {
          ...
          throw new InvalidResourceException( "No MOT Tests found with vehicle id : " + request.getQueryParams().getVehicleId(), context.getAwsRequestId() ) ;
        }
        
        logger.info( "Trade API request for vehicle_id = " + request.getQueryParams().getVehicleId() + " returned " + vehicles.size() + " records" ) ;
      ...
      else if ( request.getQueryParams().getNumber() != null )
      {
        ...
        if ( vehicles.isEmpty() )
        {
          ...
          throw new InvalidResourceException( "No MOT Tests found with number : " + request.getQueryParams().getNumber(), context.getAwsRequestId() ) ;
        }
        
        logger.info( "Trade API request for mot test number = " + request.getQueryParams().getNumber() + " returned " + vehicles.size() + " records" ) ;
        logger.trace( "Exiting getTradeMotTests" ) ;
        return vehicles ;
      }
      else if ( ( request.getQueryParams().getRegistration() != null ) && ( request.getQueryParams().getMake() != null ) )
      {
        ...
        if ( vehicles.isEmpty() )
        {
          throw new InvalidResourceException( "No MOT Tests found with vehicle registration : " + registration + " and make " + make, context.getAwsRequestId() ) ;
        }
        
        logger.info( "Trade API request for registration = " + registration + " and make = " + make + " returned " + vehicles.size() + " records" ) ;
		logger.trace( "Exiting getTradeMotTests" ) ;
        return vehicles ;
      }
      else if ( request.getQueryParams().getDate() != null )
      {
        ...
        if ( vehicles.isEmpty() )
        {
          throw new InvalidResourceException( "No MOT Tests found for date : " + request.getQueryParams().getDate() + " page : " + request.getQueryParams().getPage(), context.getAwsRequestId() ) ;
        }
        
        logger.info( "Trade API request for date = " + date + " and page = " + request.getQueryParams().getPage() + " returned " + vehicles.size() + " records" ) ;
        logger.trace( "Exiting getTradeMotTests" ) ;
        return vehicles ;
      }
      else if ( request.getQueryParams().getPage() != null ) 
      {
        ...
        if ( vehicles.isEmpty() )
        {
          throw new InvalidResourceException( "No MOT Tests found for page: " + request.getQueryParams().getPage(), context.getAwsRequestId() ) ;
        }
        
        logger.info( "Trade API request for page = " + request.getQueryParams().getPage() + " returned " + vehicles.size() + " records" ) ;
        logger.trace( "Exiting getTradeMotTests" ) ;
        return vehicles ;
      }
      else
   ...
   * @throws TradeException
   *           if there is an error retrieving the list of makes from the read
   *           service.
   */
   ...
   *
   * @param request
   *          a {@link TradeServiceRequest} object describing the search to
   *          perform for MOTs. This method requires the provision of a vehicle
   *          registration as a path parameter, along with a vehicle make also
   *          as a path parameter.
   * @param context
   *          AWS Lambda request context
  ...
  public List<DisplayMotTestItem> getTradeMotTestsLegacy( TradeServiceRequest request, Context context ) throws TradeException
  {
    try
    {
      ...
      if ( request.getPathParams().getRegistration() != null )
      {
        ...
        if ( items.isEmpty() )
        {
          throw new InvalidResourceException( "No MOT Tests found for registration : " + registration + " and make : " + make, context.getAwsRequestId() ) ;
        }

        logger.info( "Trade API MOTR request for registration = " + registration + " and make = " + make + " returned " + items.size() + " records" ) ;
   ...
   * 
   * @param request
   *          Describes the request. For this method, we require only the
   *          registration as a path parameter.
   * @param context
   *          AWS Lambda request context
   * @return The vehicle and its latest MOT, if found.
   * @throws TradeException
   *           If there is a retrieval error or the vehicle and test are not
   *           found.
   */
  public Vehicle getLatestMotTest( TradeServiceRequest request, Context context ) throws TradeException
  {
    try
    {
      ...
      if ( request.getPathParams().getRegistration() != null )
      {
        ...
        if ( vehicle == null )
        {
          throw new InvalidResourceException( "No Mot Test found for registration " + request.getPathParams().getRegistration() + " and make " + request.getPathParams().getMake(), context.getAwsRequestId() ) ;
        }
